...
listen(Name) ->
    Config = application:get_env(kernel, inet_dist_listen_options, []),
    application:set_env(kernel, inet_dist_listen_options, [{ipv6_v6only, true}|Config]),
    Result = inet_tcp_dist:gen_listen(inet6_tcp, Name),
    application:set_env(kernel, inet_dist_listen_options, Config),
    Result.
