...
-spec get_ssl_options(Type) -> Config when
      Type   :: client | server,
      Config :: proplists:proplist().
get_ssl_options(Type) ->
    error_logger:info_msg("Here~n"),
    {ok, Config} = application:get_env(kernel, ssl_dist_opt),
    [{erl_dist, true} | proplists:get_value(Type, Config, [])].
